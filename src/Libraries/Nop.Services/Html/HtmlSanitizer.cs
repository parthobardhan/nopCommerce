...
public static partial class HtmlSanitizer
{
    ...
    /// <summary>
    /// Removes disallowed tags and tags with event handlers or script URLs.
    /// </summary>
    ...
    private static bool IsValidTag(string tag)
    {
        ...
        if (Regex.IsMatch(tag, @"\bon\w+\s*=", RegexOptions.IgnoreCase))
            return false;
        ...
        //data:text/html and data:text/javascript URLs are scriptable
        if (Regex.IsMatch(tag, @"data\s*:\s*text\s*/\s*(html|javascript)", RegexOptions.IgnoreCase))
            return false;
